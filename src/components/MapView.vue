...
<template>
  ...
  <MainMenu @load-data="handleDataLoad" @load-sample="handleSampleLoad" @calculate-tool="calculate"
    @run-model="runDisasterModel" :layers="layersWithRaster" />
  <attribute-table :layers="layersWithAttributes" />
  <analysis-view :result="landslide - result" />
</template>
...
export default {
  ...
  methods: {
    initMap() {
      this.map = L.map('map', {
        center: [30.586377, 98.853130], // Adjust the center as needed
        zoom: 9,
    ...
    addRasWMSLayer() {
      ...
      if (this.isOpticalLayerAdded) {
        const cz_s2_band234 = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
    ...
    addSarWMSLayer() {
      if (this.isSarLayerAdded) {
        // 川藏研究区 GeoServer WMS 服务的 URL
        const cz_SarVH_Asc = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
    ...
    addVecWFSLayer() {
      if (this.isVectorLayerAdded) {
        ...
        const cz_fault = 'http://localhost:8080/geoserver/chuanzang/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=chuanzang%3ACN-faults_1&outputFormat=application%2Fjson';
        fetch(cz_fault)
          .then(response => response.json())
          .then(data => {
            // 使用 GeoJSON 数据创建一个图层
        ...
        const cz_reservoir = 'http://localhost:8080/geoserver/chuanzang/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=chuanzang%3Aall_reservoirs_clip&outputFormat=application%2Fjson';
        fetch(cz_reservoir)
          .then(response => response.json())
          .then(data => {
            // 使用 GeoJSON 数据创建一个图层
    ...
    addInSARLayer() {
      if (this.isInSARLayerAdded) {
        ...
        const insar_clip_26 = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        ...
        const insar_clip_128 = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        ...
        const insar_clip_172 = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        ...
        const insar_clip_99 = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        ...
        const insar_clip_70 = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        ...
        // 金沙江2023insar速率
        const insar_JinS_vel = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        ...
        // 将两个图层组合为一个图层组
        const combinedInSAR = L.layerGroup([insar_clip_26, insar_clip_128, insar_clip_172, insar_clip_99, insar_clip_70]);
        // 将图层组添加到图层控制器中
        ...
        this.layerControl.addOverlay(insar_JinS_vel, 'insar:金沙江形变速率_2023');
        this.layersWithRaster.push({
          name: '2023_金沙江平均形变速率',
        });
      } else {
      ...
      if (this.isSampleLayerAdded) {
        ...
        fetch(landslide_sample)
          ...
          .then(data => {
            ...
            // 将图层添加到图层控制器中
            this.layerControl.addOverlay(geoJsonLayer, '20km缓冲区滑坡光学样本');
            this.layersWithAttributes.push({
              name: '20km缓冲区滑坡样本',
              attributes: data.features.map(f => f.properties),
            });
    ...
    addInsarSampleLayer() {
      if (this.isInsarSampleLayerAdded) {
        // 研究区滑坡样本点 GeoServer WFS 服务的 URL
        const insar_sample = 'http://localhost:8080/geoserver/chuanzang/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=chuanzang%3AJinS_InSAR_Sample&outputFormat=application%2Fjson';
        fetch(insar_sample)
          .then(response => response.json())
          .then(data => {
            // 使用 GeoJSON 数据创建一个图层
    ...
    addSlopeLayer() {
      const slopeLayer = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
    ...
    runDisasterModel(method) {
      if (method == 'optical' && !this.isOpticalResAdded) {
        this.addOpticalRes();
        this.isOpticalResAdded = true;
      }
      else if (method == 'insar' && !this.isInsarResAdded) {
        this.addInsarRes();
        this.isInsarResAdded = true;
      }
      else if (method == 'sync' && !this.isSyncResAdded) {
        this.addSyncRes();
        this.isSyncResAdded = true;
      }
    },
    addOpticalRes() {
      const opticalLayer = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        layers: 'cztif:optical_res_256',
        format: 'image/png',
        transparent: true,
        version: '1.3.0'
      });
      // Optional: Keep track of the WMS layer in the component's data
      // This allows for easier manipulation (e.g., toggling) later on
      this.opticalLayer = opticalLayer;
      this.layerControl.addOverlay(opticalLayer, 'Optical:川藏线滑坡检测');
    },
    addInsarRes() {
      const insarLayer = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        layers: 'cztif:JinS_insar_res',
        format: 'image/png',
        transparent: true,
        version: '1.3.0'
      });
      // Optional: Keep track of the WMS layer in the component's data
      // This allows for easier manipulation (e.g., toggling) later on
      this.insarLayer = insarLayer;
      this.layerControl.addOverlay(insarLayer, 'InSAR:金沙江区域滑坡检测');
    },
    addSyncRes() {
      const syncLayer = L.tileLayer.wms("http://localhost:8080/geoserver/wms", {
        layers: 'cztif:JinS_syncth_res',
        format: 'image/png',
        transparent: true,
        version: '1.3.0'
      });
      ...
      this.layerControl.addOverlay(syncLayer, 'Overall:金沙江区域滑坡检测');
    },
